...
var Play5Layer = cc.Layer.extend({
    sprite:null,
    whack_stage :null,
    whack_sprite_Image : [],
    ...
    ctor:function (level, level_image, level_image_array) {
        ...
        no_hits = 0;
        pause_flag = true;
        cc.spriteFrameCache.addSpriteFrames(res.whackAMole_play5_plist);
        map = cc.TMXTiledMap.create(res.WhackAMole_tile5_back);
        this.addChild(map,0);
        var  map1 = cc.TMXTiledMap.create(res.WhackAMole_tile5_front);
		...
		whack_alpha_Coordinates = [{x: 40, y: 940}, {x: 920, y: 920},
									{x: 1920, y: 940}, {x: 500, y: 620},
									{x: 1420, y: 640}];
    ...
    onTouchBegan: function(touch, event){
        ...
        if (cc.rectContainsPoint(targetRectangle, location)){
                if(whack_stage == 1 && target.id ==  "0E" && pause_flag){
                             no_hits++; 
                        ...
                        else if(whack_stage == 2 && target.id ==  "0J" && pause_flag){
                             no_hits++; 
                                self.removeChild(whack_alpha_Image);
                                cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                                whack_score += 1;
                         ...
                         else if(whack_stage == 4 && target.id ==  "0R" && pause_flag){
                              no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
                          ...
                          else if(whack_stage == 5  && target.id ==  "0W" && pause_flag){
                             no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
...
var Play5Scene = cc.Scene.extend({
    ctor:function (x,y,z) {
         this._super();
         var layer = new Play5Layer(x,y,z);
         this.addChild(layer);
     }
});