...
var Play2Layer = cc.Layer.extend({
    sprite:null,
    whack_stage :null,
    whack_sprite_Image : [],
   ...
   ctor:function (level, level_image, level_image_array) {
        ...
        no_hits = 0;
        pause_flag = true;
        cc.spriteFrameCache.addSpriteFrames(res.whackAMole_play2_plist);
        map = cc.TMXTiledMap.create(res.WhackAMole_tile5);
        this.addChild(map,0);
        
        var  map1 = cc.TMXTiledMap.create(res.WhackAMole_tile5_new);
        this.addChild(map1,1);
        cc.log(whack_stage_image);
        ...
        whack_alpha_Coordinates = [{x: 160, y: 560}, {x: 1120, y: 540}, {x: 2000, y: 540}, {x: 1580, y: 300},{x: 600, y: 320}];
    ...
    showAlpha: function () {
     ...
     cc.log(whack_sprite_Image[whack_key]);
    ...
    onTouchBegan: function(touch, event){
         ...
         if (cc.rectContainsPoint(targetRectangle, location))
                   {
                     if(whack_stage == 1 && target.id ==  "0B" && pause_flag){
                             no_hits++; 
                        ...
                        else if(whack_stage == 2 && target.id ==  "0G" && pause_flag){
                             no_hits++; 
                                self.removeChild(whack_alpha_Image);
                                cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                                whack_score += 1;
                        ...
                        else if(whack_stage == 3 && target.id ==  "0L" && pause_flag){
                                no_hits++;
                                self.removeChild(whack_alpha_Image);
                                 cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                                 whack_score += 1;
                         ...
                         else if(whack_stage == 4 && target.id ==  "0O" && pause_flag){
                              no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
                          ...
                          else if(whack_stage == 5  && target.id ==  "0T" && pause_flag){
                             no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
                          ...
                          else if(whack_stage == 6 && target.id ==  "0Y" && pause_flag){
                           no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
...
var Play2Scene = cc.Scene.extend({ 
    ctor:function (x,y,z) {
         this._super();
         var layer = new Play2Layer(x,y,z);
         this.addChild(layer);
     },
});