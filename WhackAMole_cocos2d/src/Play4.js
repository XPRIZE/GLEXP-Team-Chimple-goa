...
var Play4Layer = cc.Layer.extend({
    sprite:null,
    whack_stage :null,
    whack_sprite_Image : [],
    ...
    ctor:function (level, level_image, level_image_array) {
        ...
        no_hits = 0;
        pause_flag = true;
        cc.spriteFrameCache.addSpriteFrames(res.whackAMole_play4_plist);
        map = cc.TMXTiledMap.create(res.WhackAMole_tile4_back);
        this.addChild(map,0);
        var  map1 = cc.TMXTiledMap.create(res.WhackAMole_tile4_front);
        ...
        whack_alpha_Coordinates = [{x: 40, y: 800}, {x: 1040, y: 800},
                                    {x: 2080, y: 800}, {x: 560, y: 360},
                                    {x: 1600, y: 360}];
    ...
    onTouchBegan: function(touch, event){
         ...
         if (cc.rectContainsPoint(targetRectangle, location))
                   {
                     if(whack_stage == 1 && target.id ==  "0D" && pause_flag){
                             no_hits++; 
                        ...
                        else if(whack_stage == 2 && target.id ==  "0I" && pause_flag){
                             no_hits++; 
                                self.removeChild(whack_alpha_Image);
                                cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                                whack_score += 1;
                        ...
                        else if(whack_stage == 3 && target.id ==  "0N" && pause_flag){
                                no_hits++;
                                self.removeChild(whack_alpha_Image);
                                 cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                                 whack_score += 1;
                         ...
                         else if(whack_stage == 4 && target.id ==  "0Q" && pause_flag){
                              no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
                          ...
                          else if(whack_stage == 5  && target.id ==  "0V" && pause_flag){
                             no_hits++;
                              self.removeChild(whack_alpha_Image);
                              cc.audioEngine.playEffect(res.WhackAMole_rightMusic); 
                              whack_score += 1;
...
var Play4Scene = cc.Scene.extend({
    
    ctor:function (x,y,z) {
         this._super();
         var layer = new Play4Layer(x,y,z);
         this.addChild(layer);
     },
});