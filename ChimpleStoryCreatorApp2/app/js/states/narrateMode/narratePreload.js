...
var NarratePreloadState = function (game) {
  var self = this;
  self.game = game;
};

NarratePreloadState.prototype = Object.create(Phaser.State.prototype);
NarratePreloadState.prototype.constructor = NarratePreloadState;

NarratePreloadState.prototype.init = function (indexPageToLoad) {
  var self = this;
  self.game.currentPageIndex = indexPageToLoad;
...
NarratePreloadState.prototype.preload = function () {
  var self = this;
  /*load all background images, characters and props for current story, page if exists*/
  //clean up as loading all these not required at PlayTIME
  ...
  self.game.load.image('settings', "assets/images/icons/settings.png");
  ...
  self.game.load.image('prev', 'assets/images/icons/arrow1.png');
  self.game.load.image('next', 'assets/images/icons/arrow2.png');

  self.loadIndexedPage();
};

NarratePreloadState.prototype.loadIndexedPage = function () {
  var self = this;
  var story = self.game.storageDB.findStoryId(this.game.storage_config.storiesCollection, this.game.scope.storyId);
  console.log('story:' + story);
  if (story == null) {
    console.log("Cannot play story as no pages are written");
    return;
  }
  self.game.totalStoryPages = story.pages.length;
  self.game.currentPage = _.find(story.pages, function (page, index) {
    return index == self.game.currentPageIndex || 0;
  });

  console.log('self.game.currentPage in preload:' + self.game.currentPage);
  self.game.scope.pageId = self.game.currentPage.pageId;
...
NarratePreloadState.prototype.loadUpdate = function () {
  var self = this;
  self.percentText.text = self.game.load.progress + "%";
  self.drawBar(self.game.load.progress / 100);
};

NarratePreloadState.prototype.drawBar = function (f) {
  var self = this;
  self.barMask.clear();
...
NarratePreloadState.prototype.create = function () {
  var self = this;
  self.drawBar(1);
  self.percentText.text = "100%";
  self.game.fadePlugin.fadeAndPlay("rgb(0,0,0)", 0.5, "NarrateGameState");
};

module.exports = NarratePreloadState;
