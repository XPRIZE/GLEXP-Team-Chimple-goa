...
bool Order::init()
{
	...
	for (short i = 0; i < 5; i++) {
		...
		obj1->setPositionY(visibleSize.height * 0.1 + i * (obj1->getContentSize().height ));
		obj1->setName(str1.at(i));
		obj1->setColor(Color3B(111,231,231));
		str = str + str;
		_boxes.pushBack(obj1);

	//	auto duplicateNode = Node::create();
	//	duplicateNode->setPositionX(visibleSize.width / 2);
	//	duplicateNode->setPositionY(visibleSize.height * 0.1 + i * (obj1->getContentSize().height));
	//	this->addChild(duplicateNode);
		...
		_lastBoxPosition = visibleSize.height * 0.1 + (i) * (obj1->getContentSize().height );
	...
	auto test = CSLoader::createNode("orderfarm/background.csb");
	test->setPositionX(visibleSize.width/2);
	//this->addChild(test);
	return true;
}
...
void Order::onTouchMoved(cocos2d::Touch * touch, cocos2d::Event * event)
{
	Size visibleSize = Director::getInstance()->getVisibleSize();
	auto target = event->getCurrentTarget();
	if (touch->getLocation().y > (visibleSize.height * 0.1) && (touch->getLocation().y < (_lastBoxPosition + target->getContentSize().height))) {
...
//   swaping the arrays
		...
		float num = yy / target->getContentSize().height;
		int qqq = (int)(num);
		CCLOG(" index = %d", qqq);
		...
		for (short i = 0; i < _boxes.size(); i++) {
			if (_boxes.at(i) != target) {
				...
				int index = (int)posY / target->getContentSize().height;
				if (qqq == index) {
					if (touch->getPreviousLocation().y - touch->getLocation().y > 5) {
						if (qqq == _boxes.size() - 1) {
							_boxes.at(i)->setPositionY(((qqq) *target->getContentSize().height) + (visibleSize.height* 0.1));
						} 
						else {
							_boxes.at(i)->setPositionY(((qqq + 1) *target->getContentSize().height) + (visibleSize.height* 0.1));
						}
...
void Order::onTouchEnded(cocos2d::Touch * touch, cocos2d::Event * event)
{
	...
	int qqq = (int)(num );
	CCLOG(" index = %d", qqq);
	target->setPositionY(visibleSize.height* 0.1 + ((qqq) * (target->getContentSize().height )));
}
