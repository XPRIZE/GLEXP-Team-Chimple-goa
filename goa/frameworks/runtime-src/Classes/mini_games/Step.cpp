...
void Step::onEnterTransitionDidFinish()
{
	...
	for (int i = 0; i < _position.size(); i++)
	{
		...
		if (_differentLevel[_level - 1][i] == 0)
		{
			...
			LoadingBarDetails._label = CommonLabelTTF::create("?", "Arial", 120);
			LoadingBarDetails._upLabel = CommonLabelTTF::create("?", "Arial", 120);
		...
		else
		{
			...
			LabelTTF *_label = CommonLabelTTF::create(_labelText.str(), "Arial", 120);
