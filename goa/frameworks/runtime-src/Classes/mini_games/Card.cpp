...
void Card::onEnterTransitionDidFinish()
{
	...
	LabelTTF *_boardText = CommonLabelTTF::create(_boardTextName.str(), "Arial", 120);
	...
	for (int i = 0; i < _pairCard; i++)
	{
		...
		if (i < _pairCard - 1)
		{
			LabelTTF *_boardText = CommonLabelTTF::create("+", "Arial", 120);
				...
				if (_spriteDetails.at(sprite._index)._flag == 0)
				{
					for (int i = 0; i < _toDoDetails.size(); i++)
					{
						if (_toDoDetails.at(i)._child == 0)
						{
							...
							LabelTTF *_boardText = CommonLabelTTF::create(_digit.str(), "Arial", 120);
