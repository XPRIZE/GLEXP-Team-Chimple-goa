...
=======
  ...
  set mask(maskGraphics) {
    if (this.isMask) {
      let maskA = new Shape(this.game, this._shape.relativeAnchor, this._shape.relativeOffset, this._shape.offsetInPixel, maskGraphics);
      maskA.scale = this.currentScale.clone();
      this.addChild(maskA);
      maskA.name = this.name + '_mask_' + this.getChildIndex(maskA);
      this._maskA = maskA;
      this._shape.mask = maskA;
      //maskA.positionRelativeToParent();
      maskA.dirty = true; //Due to a bug in Pixi for WebGL need to explicitly set dirty for mask true
>>>>>>> Stashed changes
    }
